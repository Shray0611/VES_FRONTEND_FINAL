...
const TemplateCard = ({ imageSrc, index }) => {
  return (
    ...
    >
      <img src={imageSrc || "/placeholder.svg"} alt={`Template ${index + 1}`} className="w-full h-full object-cover" />
      ...
      <div className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-24">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            >
              Transform your recognition process with our seamless e-certification platform for education, awards, and
              special occasions.
            </motion.p>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
        ...
        <FadeInSection>
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-[#5f4b32] mb-4">Why Choose Our Platform</h2>
            <p className="text-[#7d6954] text-lg max-w-2xl mx-auto">
              Our comprehensive solution streamlines the entire certification process
            </p>
          </div>
        ...
        <FadeInSection>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <motion.div
              ...
              transition={{ type: "spring", stiffness: 300, damping: 20 }}
            >
              <h2 className="text-3xl md:text-4xl font-bold text-[#5f4b32] mb-4">Bulk Certificate Generation</h2>
              <p className="text-[#7d6954] text-lg leading-relaxed mb-6">
                Import data directly from Excel spreadsheets to generate hundreds of personalized certificates in
                minutes. Our smart system handles all the formatting for you.
              </p>
              <ul className="space-y-3">
                {["Upload your Excel file", "Map your columns", "Choose a template", "Generate all certificates"].map(
                  (step, index) => (
                    <motion.li
                      key={index}
        ...
        <FadeInSection>
          <div className="bg-white/70 backdrop-blur-md rounded-3xl shadow-xl p-10 border border-[#e0c9a9]/30">
            <h2 className="text-3xl md:text-4xl font-semibold text-[#5f4b32] text-center mb-8">Template Gallery</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[1, 2, 3, 4].map((num, index) => (
                <TemplateCard key={num} imageSrc={`/assets/templates/certi_demo_${num}.png`} index={index} />
              ))}
            </div>
        ...
        <FadeInSection>
          <motion.div
            ...
            transition={{ type: "spring", stiffness: 300, damping: 20 }}
          >
            <h2 className="text-3xl md:text-4xl font-bold text-[#5f4b32] mb-4">Ready to Get Started?</h2>
      ...
      <footer className="bg-[#5f4b32] text-white py-8 mt-auto">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            ...
            <p className="text-white/80">Professional certificate generation and management platform</p>
          </div>

          <div>
            <h3 className="text-lg font-medium mb-4">Quick Links</h3>
            <ul className="space-y-2">
              {["Home", "Features", "Templates", "Pricing", "Contact"].map((link) => (
                <li key={link}>
        ...
        <div className="max-w-7xl mx-auto px-4 pt-6 mt-6 border-t border-white/20 text-center">
          <p className="text-sm md:text-base text-white/70">
            © {new Date().getFullYear()} VESIT-ECertificate. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
